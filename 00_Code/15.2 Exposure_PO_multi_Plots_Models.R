# 15.2 Exposure–perinatal outcome models multicontaminante — figuras Cox ----
#
# Uso:
#   Rscript "00_Code/15.2 Exposure_PO_multi_Plots_Models.R"
#
# Requiere: 02_Output/Exposure_PO_Multi/Models/Exposure_models_PO_multi_cox.xlsx
# Salida:   02_Output/Exposure_PO_Multi/Figures/
...
PO_MULTI_EXPOSURE_PLOT_LEVELS <- c(
...
auto_panel_y_axis <- function(
  ...
  limits <- c(center - half_span, center + half_span)
  if (!is.null(y_min) && limits[[1L]] < y_min) {
    limits[[1L]] <- y_min
  }
  if (!is.null(y_max) && limits[[2L]] > y_max) {
    limits[[2L]] <- y_max
  }
...
fixed_panel_y_axis <- function(y_min, y_max, by = 1) {
  breaks <- seq(y_min, y_max, by = by)
  list(
...
PO_MULTI_IQR_Y_AXIS <- fixed_panel_y_axis(y_min = -3, y_max = 4, by = 1)
...
build_preterm_figure <- function(plot_data_cox, scale_key, out_fname) {
  plot_data_cox <- plot_data_cox |>
    ...
    dplyr::mutate(
      exposure = factor(.data$exposure, levels = PO_MULTI_EXPOSURE_PLOT_LEVELS),
      adjustment = factor(.data$adjustment, levels = c("Unadjusted", "Adjusted"))
    )

  plots <- lapply(seq_along(PO_MULTI_EXPOSURE_POLLUTANTS), function(i) {
    cont <- PO_MULTI_EXPOSURE_POLLUTANTS[[i]]
    data_panel <- plot_data_cox |> dplyr::filter(.data$contaminante == cont)
    y_axis <- if (identical(scale_key, "iqr")) {
      if (identical(cont, "o3")) {
        PO_MULTI_IQR_Y_AXIS
      } else {
        auto_panel_y_axis(
          data_panel,
...
plot_data_all <- prepare_po_multi_plot_table_data(models_cox)
