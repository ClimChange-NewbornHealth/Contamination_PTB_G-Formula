# 14.1 G-Formula multicontaminante — construcción de intervenciones (pct20 × 3) ----
#
# Genera RDS en 02_Output/G-Form-Multi/Interventions/ (no sobrescribe G-Form).
# Requisito previo: datos de cohorte (mismos insumos que 10.0).
#
# Uso:
#   Rscript "00_Code/14.1 G-Form_multi_build_interventions.R"
#   GFORM_EXEC_MODE=server Rscript "00_Code/14.1 G-Form_multi_build_interventions.R"
...
overwrite_interventions <- gform_env_bool("GFORM_MULTI_OVERWRITE", default = FALSE)
...
message("=== G-Formula Multi Etapa 1: construcción de intervenciones ===")
message("Modo: ", execution_mode)
message("Destino: ", dir_interventions)
message("Escenarios: ", length(GFORM_MULTI_INTERVENTION_REGISTRY), " (pct20 × 3 contaminantes)")
if (run_parallel) {
  gform_setup_parallel(task = "build", config = parallel_config)
  message("Construcción paralela.")
} else {
...
built <- build_all_interventions(
  data_long = data_long,
  registry = GFORM_MULTI_INTERVENTION_REGISTRY,
...
gform_print_timing_summary(timing_log, "Etapa 1 Multi — construcción de intervenciones")
