...
Para la persona $i$ y la semana gestacional $w$, denotamos $X_{iw}$ la concentración del contaminante modelado (p. ej. $PM_{2.5}$ en µg/m³).
...
$$
h_i(u \mid t) = h_{0t}(u) \exp(\eta_{it})
$$
...
### 4.1 Incremento del hazard de línea base

Del modelo Cox de la semana $t$, se extrae la función de riesgo acumulada de línea base $\hat{H}_{0t}(u)$. El incremento entre $t-1$ y $t$:
...
$$
\hat{p}_i(t \mid s) = 1 - \exp(-\Delta \hat{H}_{0t}(t) \cdot \exp(\widehat{\mathrm{lp}}_{it}^{(s)}))
$$
...
**Supervivencia (no pretérmino hasta $t$):**
...
$$
\tilde{\boldsymbol{\beta}}_t^{(b)} \sim \mathcal{N}(\hat{\boldsymbol{\beta}}_t, \widehat{\mathrm{Var}}(\hat{\boldsymbol{\beta}}_t))
$$
...
$$
[ Q_{0{,}025}(\hat{\theta}^{(1)}, …, \hat{\theta}^{(B)}), Q_{0{,}975}(\hat{\theta}^{(1)}, …, \hat{\theta}^{(B)}) ]
$$
