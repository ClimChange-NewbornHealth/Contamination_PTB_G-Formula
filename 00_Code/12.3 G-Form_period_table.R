# 12.3 G-Formula por períodos — tabla resumen de efectos poblacionales ----
#
# Uso (desde la raíz del proyecto):
#   Rscript "00_Code/12.3 G-Form_period_table.R"
#
# Entrada:
#   02_Output/G-Form-Period/Summary_results/{stub}_point_estimates.xlsx
#     stubs: pm25/no2/o3 _pct10/_pct20/_pct30, _lt20, _lt15, _lt10, _lt5
#            con sufijo opcional _tri / _full (alcance período)
#   02_Output/G-Form-Period/Bootstrap/{stub}/population_boot.csv  (IC de PAF si existe)
# Salida:
#   02_Output/G-Form-Period/Summary_results/Table_population_effects_summary.xlsx
#
# Misma estructura que 10.4 G-Form_table.R, con columna "Time Intervention"
# (Trimester / Overall) y reordenamiento por alcance temporal dentro de cada
# contaminante. Omite intervenciones sin Excel local.
# La tabla presenta prevalencia, RD, AR y PAF en escala % (×100);
# Cases y Risk Ratio permanecen en unidades originales.
...
repair_point_estimates_stub <- function(output_stub) {
  ...
  if (file.exists(excel_path)) {
    ...
    repair_sheets <- "population_effects"
...
resolve_application_scope <- function(application_scope, stub) {
  if (!is.na(application_scope) && nzchar(application_scope)) {
...
resolve_time_intervention <- function(application_scope, stub) {
  scope <- resolve_application_scope(application_scope, stub)
  if (identical(scope, "trimester")) {
    return("Trimester")
  }
...
period_stub_candidates <- function(base_stub) {
  unique(c(
    paste0(base_stub, "_tri"),
    paste0(base_stub, "_full"),
    base_stub
  ))
}
...
discover_available_scenarios <- function(scenarios) {
  ...
  for (sc in scenarios) {
    tri_stub <- paste0(sc$stub, "_tri")
    full_stub <- paste0(sc$stub, "_full")
    base_stub <- sc$stub

    if (file.exists(scenario_excel_path(tri_stub))) {
      add_scenario(tri_stub, sc$label, "Trimester")
    } else if (file.exists(scenario_excel_path(base_stub))) {
      meta <- read_scenario_metadata(scenario_excel_path(base_stub))
      if (identical(resolve_application_scope(meta$application_scope, base_stub), "trimester")) {
        add_scenario(base_stub, sc$label, "Trimester")
      }
    }

    if (file.exists(scenario_excel_path(full_stub))) {
      add_scenario(full_stub, sc$label, "Overall")
    } else if (file.exists(scenario_excel_path(base_stub)) && !(base_stub %in% seen_stubs)) {
      meta <- read_scenario_metadata(scenario_excel_path(base_stub))
      if (identical(resolve_application_scope(meta$application_scope, base_stub), "full")) {
        add_scenario(base_stub, sc$label, "Overall")
      }
    }
  }
...
build_pollutant_block <- function(spec) {
  ...
  if (!length(available)) {
    for (sc in spec$scenarios) {
      for (stub in period_stub_candidates(sc$stub)) {
...
message("Construyendo tabla resumen de efectos poblacionales (período)...")
