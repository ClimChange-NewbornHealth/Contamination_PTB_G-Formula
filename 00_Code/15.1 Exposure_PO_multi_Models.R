# 15.1 Exposure–perinatal outcome models multicontaminante — estimación Cox ----
#
# Modelos Cox multicontaminante por contaminante focal (PM2.5, NO2, O3; kriging):
#   - período completo (tot): focal overall + overall de co-contaminantes
#   - trimestres (t1+t2+t3): focal trimestral + trimestres de co-contaminantes
# Escalas: raw e IQR
#
# Uso (desde la raíz del proyecto):
#   Rscript "00_Code/15.1 Exposure_PO_multi_Models.R"
#
# Salida:
#   02_Output/Exposure_PO_Multi/Models/Exposure_models_PO_multi_cox.xlsx
#   02_Output/Exposure_PO_Multi/Models/Exposure_models_PO_multi_cox.RData
#   02_Output/Exposure_PO_Multi/Models/List_models_exposure_PO_multi.xlsx
...
source("00_Code/15.0 Exposure_PO_multi_functions.R")

data_inp <- "01_Data/Output/"
data_out <- "02_Output/Exposure_PO_Multi/"
...
required_full_tri <- unlist(lapply(
  PO_MULTI_EXPOSURE_POLLUTANTS,
  function(p) {
    c(
      po_multi_exposure_var(p, "tot", "raw"),
      po_multi_exposure_var(p, "tot", "iqr"),
      po_multi_exposure_var(p, "t1", "raw"),
      po_multi_exposure_var(p, "t2", "raw"),
      po_multi_exposure_var(p, "t3", "raw"),
      po_multi_exposure_var(p, "t1", "iqr"),
      po_multi_exposure_var(p, "t2", "iqr"),
      po_multi_exposure_var(p, "t3", "iqr")
    )
  }
))
...
combinations <- build_po_multi_model_grid(
...
writexl::write_xlsx(
  combinations,
  path = file.path(dir_models, "List_models_exposure_PO_multi.xlsx")
)
message("Grilla de modelos multicontaminante: ", nrow(combinations), " especificaciones Cox.")

## 3. Estimación Cox ----

run_po_multi_model_grid <- function(combinations, fit_fun, label) {
...
results_list_cox <- run_po_multi_model_grid(
  combinations,
  fit_fun = fit_po_multi_cox_model,
  label = "Cox multicontaminante"
)
...
save(
  results_list_cox,
  file = file.path(dir_models, "Exposure_models_PO_multi_cox.RData")
)

writexl::write_xlsx(
  list(cox_models = results_cox),
  path = file.path(dir_models, "Exposure_models_PO_multi_cox.xlsx")
)
