# 14.2 G-Formula multicontaminante — corrida de intervenciones (pct20 × 3) ----
#
# Requisito previo: 14.1 G-Form_multi_build_interventions.R
#
# Uso:
#   bash "00_Code/run_gform_multi_server.sh"
#   Rscript "00_Code/14.2 G-Form_multi_models.R"
...
source("00_Code/14.0 G-Form_multi_functions.R")

data_inp <- "01_Data/Output/"
data_out_g <- GFORM_MULTI_DATA_OUT
...
intervention_env <- gform_env_int_vec("GFORM_MULTI_INTERVENTIONS", NULL)
...
intervention_numbers <- if (!is.null(intervention_env)) {
  ...
  seq_len(length(GFORM_MULTI_INTERVENTION_ORDER))
}
...
run_one_gform_multi_intervention <- function(intervention_number, shared_data) {
  timing_log <- gform_timing_log_init()
  on.exit(gform_multi_clear_active(), add = TRUE)

  registry_key <- resolve_gform_multi_intervention(intervention_number)
  spec <- GFORM_MULTI_INTERVENTION_REGISTRY[[registry_key]]
  ...
  block <- gform_time_block(
    paste0("Cargar exposición semanal — ", pollutant, " + co-contaminantes"),
    load_pollutant_long_multi(pollutant, ids = data_base$id)
  )
  timing_log <- gform_timing_log_add(timing_log, block$timing)
  data_long_weekly <- block$result

  block <- gform_time_block("Co-exposiciones semanales (controles multicontaminante)", {
    build_wide_copollutants_obs(
      data_long_weekly,
      primary = pollutant,
  ...
  timing_log <- gform_timing_log_add(timing_log, block$timing)
  gform_multi_set_active(pollutant, block$result)
  ...
  block <- gform_time_block(
    paste0("Cox multicontaminante — curso natural ", pollutant, " (cache: ", dir_models, ")"),
  ...
  if (!file.exists(intervention_path)) {
    stop("RDS no encontrado (ejecutar 14.1 primero): ", intervention_path)
  }
  ...
  metadata <- list(
    ...
    model_type = "coxph_multicontaminant",
...
message("\n=== G-Formula MULTI corrida (modo: ", execution_mode, ") ===")
message("Salida: ", data_out_g)
...
print_gform_multi_intervention_menu()

pollutants_needed <- unique(vapply(intervention_numbers, function(n) {
  GFORM_MULTI_INTERVENTION_REGISTRY[[resolve_gform_multi_intervention(n)]]$pollutant
}, character(1)))
...
for (int_num in intervention_numbers) {
  ...
  spec <- GFORM_MULTI_INTERVENTION_REGISTRY[[resolve_gform_multi_intervention(int_num)]]
  ...
  result <- tryCatch(
    run_one_gform_multi_intervention(int_num, shared_data),
    error = function(e) {
