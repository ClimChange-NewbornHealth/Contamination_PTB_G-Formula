...
fit_cox_model <- function(
  ...
  # Filter data with valid values in dependent and time variables
  data_subset <- data |>
  ...
  # Keep only terms available in data for current model
  rhs_terms <- trimws(stringr::str_split(rhs, " \\+ ")[[1]])
  rhs_terms <- rhs_terms[rhs_terms %in% names(data_subset)]
  ...
  # Extract all model coefficients (HR when exponentiate = TRUE)
  ...
  if (nrow(tbl) > 0) {
    tbl_out <- tbl |>
      ...
      dplyr::select(term, estimate, conf.low, conf.high, hr, hr_conf.low, hr_conf.high,
                    log_hr, log_hr_conf.low, log_hr_conf.high,
                    std.error, statistic, p.value, dependent_var, predictor,
                    tiempo, contaminante, tipo, n)
  } else {
    tbl_out <- data.frame()
  }
