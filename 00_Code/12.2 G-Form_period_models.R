# 12.2 G-Formula por períodos — modelos ajustados (servidor / local) ----
#
# Requisito previo: 12.1 G-Form_period_build_interventions.R
#
# Modelos Cox (por contaminante, cacheados):
#   - trimester (_tri): T1+T2+T3 mutuamente ajustados + covariables
#   - full (_full): exposición full + covariables
#
# Salida mínima: efectos poblacionales (prevalencia, riesgo, RD, RR, PAF).
# Bootstrap opcional (GFORM_RUN_BOOTSTRAP=true).
...
output_subdir <- trimws(Sys.getenv("GFORM_PERIOD_OUTPUT_SUBDIR", unset = ""))
if (nzchar(output_subdir)) {
  data_out_g <- file.path("02_Output/G-Form-Period", output_subdir)
  message("Outputs período en subdirectorio: ", data_out_g)
}

dir_interventions <- file.path("02_Output/G-Form-Period", "Interventions")
...
intervention_env <- gform_env_int_vec("GFORM_PERIOD_INTERVENTIONS", NULL)
...
intervention_numbers <- if (!is.null(intervention_env)) {
  ...
  seq_len(length(GFORM_PERIOD_INTERVENTION_ORDER))
}
...
boot_seed <- as.integer(gform_env_num("GFORM_BOOT_SEED", GFORM_PERIOD_DEFAULTS$boot_seed))
skip_completed <- gform_env_bool("GFORM_SKIP_COMPLETED", default = TRUE)

max_follow_up <- GFORM_PERIOD_DEFAULTS$max_follow_up
risk_weeks <- GFORM_PERIOD_DEFAULTS$risk_weeks
risk_entry_week <- GFORM_PERIOD_DEFAULTS$risk_entry_week
follow_up_weeks <- GFORM_PERIOD_DEFAULTS$follow_up_weeks
population_week <- GFORM_PERIOD_DEFAULTS$population_week
control_vars <- GFORM_PERIOD_CONTROL_VARS
dependent_var <- GFORM_PERIOD_DEFAULTS$dependent_var
baseline_scenario <- GFORM_PERIOD_DEFAULTS$baseline_scenario

run_one_period_intervention <- function(intervention_number, shared_data) {
  timing_log <- gform_timing_log_init()

  registry_key <- resolve_gform_period_intervention(intervention_number)
  spec <- GFORM_PERIOD_INTERVENTION_REGISTRY[[registry_key]]
  pollutant <- spec$pollutant
  application_scope <- spec$application_scope
  exposure_vars <- gform_period_exposure_var_names(pollutant, application_scope)
  ...
  exposure_natural <- get_period_exposure_natural(
  ...
  block <- gform_time_block(
    paste0(
      "Cox curso natural — ", pollutant, " / ", application_scope,
      " (cache: ", dir_models, ")"
    ),
    load_or_fit_period_natural_course_models(
      pollutant = pollutant,
  ...
  intervention_path <- gform_period_intervention_rds_path(
    spec$intervention_id,
    dir_interventions
  )
  if (!file.exists(intervention_path)) {
    stop("RDS no encontrado (ejecutar 12.1 primero): ", intervention_path)
  }
  ...
  metadata <- list(
    ...
    boot_iter = boot_iter,
    model_type = "coxph_period_aggregate",
...
message("\n=== G-Formula Período — corrida (modo: ", execution_mode, ") ===")
print_gform_period_cox_specs()
...
print_gform_period_intervention_menu()

batch_timing <- gform_timing_log_init()

block <- gform_time_block("Cargar cohorte período", {
  births_df <- prepare_period_births_cohort(data_inp)
  if (!is.null(sample_frac)) {
    set.seed(sample_seed)
    ids_keep <- sample(unique(births_df$id), size = floor(sample_frac * length(unique(births_df$id))))
...
block2 <- gform_time_block("Inicializar cache de exposiciones naturales", {
  list()
})
...
gform_print_timing_summary(batch_timing, "Preparación del lote período")
...
for (int_num in intervention_numbers) {
  ...
  spec <- GFORM_PERIOD_INTERVENTION_REGISTRY[[resolve_gform_period_intervention(int_num)]]
  if (skip_completed && gform_period_intervention_is_complete(
  ...
  result <- tryCatch(
    run_one_period_intervention(int_num, shared_data),
    error = function(e) {
