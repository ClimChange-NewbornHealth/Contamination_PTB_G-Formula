# 15.3 Exposure–perinatal outcome models multicontaminante — tablas Cox ----
#
# Uso:
#   Rscript "00_Code/15.3 Exposure_PO_multi_Tables_Models.R"
#
# Requiere: 02_Output/Exposure_PO_Multi/Models/Exposure_models_PO_multi_cox.xlsx
# Salida:   02_Output/Exposure_PO_Multi/Tables/Tab_Exposure_PO_multi.xlsx
#           02_Output/Exposure_PO_Multi/Tables/Tab_Exposure_PO_multi_IQR.xlsx
...
path_results <- file.path(dir_models, "Exposure_models_PO_multi_cox.xlsx")
if (!file.exists(path_results)) {
  stop("Ejecute primero 15.1 Exposure_PO_multi_Models.R")
}

models_cox <- readxl::read_excel(path_results, sheet = "cox_models")

prepare_po_multi_table_data <- function(models_df) {
  models_df |>
    dplyr::filter(
    ...
    dplyr::mutate(
      exposure = dplyr::case_when(
        ...
        model_type == "t1_t2_t3_multi" & dplyr::row_number() == 1L ~ "T1",
        model_type == "t1_t2_t3_multi" & dplyr::row_number() == 2L ~ "T2",
        model_type == "t1_t2_t3_multi" & dplyr::row_number() == 3L ~ "T3",
        TRUE ~ NA_character_
      )
    ) |>
    dplyr::ungroup() |>
...
build_table_all <- function(data_all) {
  data_all <- data_all |>
    dplyr::mutate(
      ...
      value_fmt = format_po_multi_effect_ci(.data$estimate, .data$conf.low, .data$conf.high),
...
write_scale_tables <- function(scale, suffix = "") {
  table_data_cox <- prepare_po_multi_table_data(models_cox) |>
  ...
  out_path <- file.path(
    dir_tables,
    if (nzchar(suffix)) {
      paste0("Tab_Exposure_PO_multi", suffix, ".xlsx")
    } else {
      "Tab_Exposure_PO_multi.xlsx"
    }
  )
